...
public class ArduinoFirmata {
    ...
    public void connect() throws IOException, InterruptedException {
        ...
        if (th_receive == null) {
            th_receive = new Thread(new Runnable() {
                public void run() {
                    while (isOpen()) {
                        try {
    ...
    public void analogWrite(int pin, int value) {
        byte[] writeData = { SET_PIN_MODE, (byte) pin, PWM, (byte) (ANALOG_MESSAGE | (pin & 0x0F)),
                (byte) (value & 0x7F), (byte) (value >> 7) };
        write(writeData);
    }
        ...
        if (parsingSysex) {
            ...
            if (executeMultiByteCommand != 0 && waitForData == 0) {
                ...
                case DIGITAL_MESSAGE:
                    ...
                    break;
                case ANALOG_MESSAGE:
                    setAnalogInput(multiByteChannel,
                            (storedInputData[0] << 7) + storedInputData[1]);
                    break;
                case REPORT_VERSION:
